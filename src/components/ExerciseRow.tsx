...
export function ExerciseRow({
  ...
  const isRepsPredicted =
    prediction !== null && reps === String(prediction.reps);
    ...
    <div className="bg-[var(--surface)] rounded-lg p-4 space-y-4">
      ...
      <div className="flex gap-4 cursor-pointer" onClick={onClick}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {meta && (
            <div className="text-sm text-[var(--text-muted)]">{meta}</div>
          )}
          <div className="mt-1 text-sm text-[var(--text-muted)]">
            {todaySets.length > 0 ? (
              ...
              <span>
                Suggested: {prediction.kg}kg × {prediction.reps}
              </span>
            ) : (
              <span>First set</span>
            )}
